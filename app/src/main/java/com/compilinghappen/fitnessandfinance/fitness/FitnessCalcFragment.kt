...
class FitnessCalcFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val sexAdapter = ArrayAdapter(requireContext(), R.layout.support_simple_spinner_dropdown_item, sexItems)
        binding.sexField.adapter = sexAdapter

        val activeItems = listOf<String>("Сидячий образ жизни", "Лёгкая (1-3 раза в неделю)", "Средняя (3-5 раз в неделю)", "Высокая (6-7 раз в неделю)", "Экстремальная (больше 1 раза в день)")
        val activeAdapter = ArrayAdapter(requireContext(), R.layout.support_simple_spinner_dropdown_item, activeItems)
        binding.activeField.adapter = activeAdapter

        val targetItems = listOf<String>("Сбросить вес", "Поддерживать форму", "Набрать вес")
        val targetAdapter = ArrayAdapter(requireContext(), R.layout.support_simple_spinner_dropdown_item, targetItems)
